...
server <- function(input, output, session) {
  ...
  req(seurat_obj(), input$gene_title)
  seurat_data <- seurat_obj()
  FeaturePlot_scCustom(seurat_data, features = paste0(input$gene_title, "1"))
  })

  vln_plot_reactive <- reactive({
  req(seurat_obj(), input$vln_variable, input$gene_title)
  seurat_data <- seurat_obj()
  VlnPlot2(seurat_data,
           features = paste0(input$gene_title, "1"),
           assay = "RNA",
           group.by = input$vln_variable,
           split.by = input$split_variable,
